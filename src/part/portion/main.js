...
function setup(app, container)
{
    ...
    function do_delete(bounds)
    {
        ...
        for(let component of components)
        {
            ...
            if(component.x <= bounds.right && component.x + rotatedSize.x > bounds.left &&
                component.y <= bounds.bottom && component.y + rotatedSize.y > bounds.top)
            {
            ...
            for(let y = bounds.top; y <= bounds.bottom; y++)
            {
                for(let x = bounds.left; x <= bounds.right; x++)
                {
                    ...
                    for(let side = 1; side <= 8; side <<= 1)
                    {
                        ...
                        if(app.world.loadedWorld.map.isValid(absolute.x, absolute.y) && !(absolute.x <= bounds.right && absolute.x >= bounds.left &&
                            absolute.y <= bounds.bottom && absolute.y >= bounds.top))
                        {
    ...
    function do_copy(from_bounds, position)
    {
        ...
        let offset = {x: position.x - from_bounds.left, y: position.y - from_bounds.top};
        ...
        for(let component of components)
        {
            ...
            if(component.x <= from_bounds.right && component.x + rotatedSize.x > from_bounds.left &&
                component.y <= from_bounds.bottom && component.y + rotatedSize.y > from_bounds.top)
            {
            ...
            for(let y = from_bounds.top; y <= from_bounds.bottom; y++)
            {
                for(let x = from_bounds.left; x <= from_bounds.right; x++)
                {
                    ...
                    for(let side = 1; side <= 8; side <<= 1)
                    {
                        ...
                        if(!(absolute.x <= from_bounds.right && absolute.x >= from_bounds.left &&
                            absolute.y <= from_bounds.bottom && absolute.y >= from_bounds.top))
                        {
